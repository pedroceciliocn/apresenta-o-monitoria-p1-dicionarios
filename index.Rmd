...
---
### Usando os métodos `items()`, `keys()` e `values()`
Outro método útil é o `items()`. Ele é um _gerador_ (assim como `keys` e `values`), que nesse caso, retorna uma lista com _tuplas_ contendo a chave e o valor de cada item do dicionário. Usando apenas um `for` com duas variáveis (por exemplo: chave e valor) podemos receber esses valores em uma "andada" só.
...
---
### Tuplas
Também podemos usar tuplas para atribuir e trocar valores de variáveis de maneira simultânea:
```{python, collapse = TRUE}
a, b = 10, 20 # atribuindo
print(a, b)
a, b = b, a # trocando
print(a, b)
```
--
Que seria uma opção sem a necessidade do uso de variáveis auxiliares pra salvar os valores durante a troca.
